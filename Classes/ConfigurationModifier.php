...
class ConfigurationModifier
{
    ...
    protected static function findPagesWithPlugins(array $withPlugins): array
    {
        ...
        $contentElementRecords = $queryBuilder
            ...
            ->where(
                ...
                $queryBuilder->expr()->in('list_type', $queryBuilder->createNamedParameter($withPlugins, ArrayParameterType::STRING))
            )
            ->executeQuery()
            ->fetchFirstColumn();
    ...
    protected static function findPagesContainingModules(array $modules): array
    {
        ...
        $pageRecords = $queryBuilder
            ...
            ->from('pages')
            ->where(
                $queryBuilder->expr()->in('module', $queryBuilder->createNamedParameter($modules, ArrayParameterType::STRING))
            )
            ->executeQuery()
            ->fetchFirstColumn();
        return $pageRecords;
        ...
        $pageRecords = $queryBuilder
            ...
            ->where(
                $queryBuilder->expr()->in(
                    'doktype',
                    $queryBuilder->createNamedParameter($doktypes, ArrayParameterType::INTEGER)
                )
            )
            ->executeQuery()
            ->fetchFirstColumn();
    ...
    protected static function findPagesWithCType(array $withCType): array
    {
        ...
        $contentElementRecords = $queryBuilder
            ...
            ->from('tt_content')
            ->where(
                $queryBuilder->expr()->in('CType', $queryBuilder->createNamedParameter($withCType, ArrayParameterType::STRING))
            )
            ->executeQuery()
            ->fetchFirstColumn();
        return $contentElementRecords;
